...
public class CharacterMotor : MonoBehaviour {
    ...
    private void UpdateFunction() {
        ...
        Vector3 oldHVelocity = new Vector3(velocity.x, 0f, velocity.z);
        movement.velocity = (tr.position - lastPosition) / Time.deltaTime;
        Vector3 newHVelocity = new Vector3(movement.velocity.x, 0f, movement.velocity.z);
    ...
    private Vector3 ApplyGravityAndJumping(Vector3 velocity) {
        ...
        if (inputJump && jumping.lastButtonDownTime < 0f && canControl) jumping.lastButtonDownTime = Time.time;
    ...
    private IEnumerator SubtractNewPlatformVelocity() {
        ...
        if (movingPlatform.enabled && (movingPlatform.movementTransfer == MovementTransferOnJump.InitTransfer || movingPlatform.movementTransfer == MovementTransferOnJump.PermaTransfer)) {
            ...
            if (movingPlatform.newPlatform) {
                ...
                yield return new WaitForFixedUpdate();
                if (grounded && platform == movingPlatform.activePlatform) yield break;
            }
            movement.velocity -= movingPlatform.platformVelocity;
        }
    }
